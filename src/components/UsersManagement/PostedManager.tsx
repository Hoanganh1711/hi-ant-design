import { MailOutlined, MoneyCollectOutlined, PhoneOutlined, UserOutlined } from '@ant-design/icons'
import { Col, Divider, Row } from 'antd'
import React from 'react'

const PostedManager = () => {


  return (
    <Row style={{ justifyContent: "space-between", backgroundColor: "#fff", padding: 20, borderRadius: 10 }}>
      <Col span={10} className='user-items'>
        <Row>
          <Col className='avatar' style={{textAlign: "center", fontSize: 64}}>
            {/* <img style={{ width: 100 }} src='data:image/png;base64,'></img> */}
            <UserOutlined />
            <div style={{ fontSize: 10, textAlign: "center", marginTop: 10, padding: 4, borderRadius: 5, border: "solid 1px #000" }}>Nâng cấp Pro</div>
          </Col>

          <Col style={{ marginLeft: 30 }} className='info'>
            <div>
              <Row>
                <h3>Anh Nguyễn Hoàng</h3>
                <p
                  style={{ backgroundColor: "#27406d", color: "#fff", borderRadius: 50, padding: "5px", marginLeft: 10 }}
                >
                  Happy Basic
                </p>
              </Row>
            </div>
            <div className='icons-group'>
              <MoneyCollectOutlined />
              <span>0</span>
              <div style={{ marginTop: 5 }}>
                <UserOutlined />
              </div>
              <div style={{ marginTop: 5 }}>
                <PhoneOutlined />
                <span style={{ marginLeft: 5 }}>0367093593</span>
              </div>
              <div style={{ marginTop: 5 }}>
                <MailOutlined />
                <span style={{ marginLeft: 5 }}>hoanggiaanh171193@gmail.com</span>
              </div>
            </div>
          </Col>
        </Row>
      </Col>

      <Col span={14}>
        <Row className='buttoms-group' style={{ justifyContent: "right" }}>
          <div style={{ color: "#fff", backgroundColor: "#0298da", padding: "5px 20px", border: "none", borderRadius: 5 }}>Đăng tin</div>
          <div style={{ color: "#000", padding: "5px 20px", border: "solid 1px #000", borderRadius: 5, marginLeft: 20 }}>Nạp tiền</div>
          <div style={{ color: "#000", padding: "5px 20px", border: "solid 1px #000", borderRadius: 5, marginLeft: 20 }}>Đổi điểm</div>
        </Row>
        <Divider />
        <Row style={{ justifyContent: "space-between" }}>
          <div style={{fontSize: 15}}>Tài khoản (VNĐ)<p style={{fontSize: 20, color: "#0298da", fontWeight: 600}}>0</p></div>
          <div style={{fontSize: 15}}>Coupon khả dụng<p style={{fontSize: 20, color: "#0298da", fontWeight: 600}}>5</p></div>
          <div style={{fontSize: 15}}>Điểm tiêu dùng 1<p style={{fontSize: 20, color: "#0298da", fontWeight: 600}}>0</p></div>
          <div style={{fontSize: 15}}>Điểm tiểu dùng 2<p style={{fontSize: 20, color: "#0298da", fontWeight: 600}}>0</p></div>
        </Row>
      </Col >
    </Row >
  )
}

export default PostedManager